    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card className="rounded-2xl border border-champagne-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-plum-900">{activeAssets.length}</p>
              <p className="text-xs text-plum-500">Active Assets</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border border-champagne-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-plum-500">Total Value</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border border-champagne-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-plum-500">High Priority</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border border-champagne-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-plum-900">{archivedAssets.length}</p>
              <p className="text-xs text-plum-500">Archived</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      {isLoading ? (
        <Card className="rounded-3xl border border-champagne-200 bg-white shadow-sm">
          <p className="py-12 text-center text-sm text-plum-500">Loading assets...</p>
        </Card>
      ) : assets.length === 0 ? (
        <Card className="rounded-3xl border border-champagne-200 bg-white shadow-sm">
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-plum-900">No digital assets yet</h3>
        ...
        <div className="space-y-6">
          {/* Active Assets */}
          <div>
            <h2 className="mb-4 text-lg font-semibold text-plum-900">Active Assets</h2>
              ...
              {activeAssets.map((asset) => {
                ...
                return (
                  <Card key={asset.id} className="rounded-2xl border border-champagne-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h3 className="font-semibold text-plum-900 truncate">{asset.name}</h3>
                            {asset.important && (
                              <Star className="h-4 w-4 text-amber-500 flex-shrink-0 fill-current" />
                            )}
                          </div>
                          {asset.platform && (
                            <p className="mt-1 text-sm text-plum-500 truncate">{asset.platform}</p>
                          )}
                    ...
                    {asset.renewalDate && (
                      <div className="mt-4 flex items-center gap-2 text-xs text-plum-500">
      ...
      >
        <div className="space-y-5">
              ...
              {categoryOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-plum-900 block">{option.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-plum-700 mb-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="important" className="text-sm font-medium text-plum-700 cursor-pointer">
