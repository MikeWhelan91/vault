    ...
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-graphite-600">
          {tier === 'free'
            ? 'Upgrade to Plus for more storage and unlimited bundles'
      ...
      {tier === 'free' && (
        <Card className="bg-gradient-to-r from-primary-50 to-secondary-50 border-primary-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-semibold text-graphite-900 mb-2">Your Current Usage</h3>
              <div className="space-y-1 text-sm text-graphite-600">
                <div>Storage: {formatBytes(metadata.totalSize)} of {tierLimits.storage.display} ({storagePercentage.toFixed(0)}%)</div>
                <div>Items: {metadata.items.length} items</div>
              </div>
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        {/* Free Tier */}
        <Card className={`border-2 ${tier === 'free' ? 'border-primary-500 bg-primary-50/30' : 'border-graphite-200'}`}>
          <div className="p-6">
            {tier === 'free' && (
              <div className="mb-4">
        ...
        {/* Plus Tier */}
        <Card className={`border-2 ${tier === 'plus' ? 'border-primary-500 bg-primary-50/30' : 'border-primary-500'} relative overflow-hidden`}>
          {tier === 'free' && (
