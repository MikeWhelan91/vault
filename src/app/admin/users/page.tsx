  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-primary-50 flex items-center justify-center">
        <Card>
          <p className="text-warm-900">Loading users...</p>
        </Card>
      </div>
    );
  }
      ...
      <header className="border-b border-warm-200 bg-white/95 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-semibold text-warm-900">User Management</h1>
        ...
        <Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-warm-200">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">User</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Tier</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Storage</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Items</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Bundles</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Heartbeat</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Joined</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="border-b border-warm-100 hover:bg-warm-50">
                    <td className="py-3 px-4">
                      <div>
                        <div className="font-medium text-warm-900">{user.email}</div>
                        <div className="text-sm text-warm-600">{user.name || 'No name'}</div>
                        <div className="text-xs text-warm-500 font-mono">{user.id}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span
                    ...
                    <td className="py-3 px-4">
                      <div>
                        ...
                        <div className="w-full bg-warm-200 rounded-full h-2 mt-1">
                          <div
                    ...
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">{user._count.items}</div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">{user._count.releaseBundles}</div>
                    </td>
                    <td className="py-3 px-4">
                      {user.heartbeat ? (
                        <div>
                        ...
                        <span className="text-sm text-warm-500">Not configured</span>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">
          ...
          {filteredUsers.length === 0 && (
            <div className="text-center py-8">
              <p className="text-warm-600">No users found matching your criteria.</p>
            </div>
          )}
        </Card>
      </div>
