  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-primary-50 flex items-center justify-center">
        <Card>
          <p className="text-warm-900">Loading bundles...</p>
        </Card>
      </div>
    );
  }
      ...
      <header className="border-b border-warm-200 bg-white/95 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-semibold text-warm-900">Bundle Management</h1>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="px-4 py-2 border border-warm-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-6">
          ...
          <Card>
            <div className="text-center">
              ...
              <div className="text-sm text-warm-600">Released</div>
            </div>
          </Card>
          <Card>
            <div className="text-center">
              ...
              <div className="text-sm text-warm-600">Pending</div>
            </div>
          </Card>
          <Card>
            <div className="text-center">
              ...
              <div className="text-sm text-warm-600">Expired</div>
            </div>
          </Card>
        </div>
        ...
        <Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-warm-200">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Bundle</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Owner</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Mode</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Status</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Items</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Trustees</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Release Date</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-warm-700">Created</th>
                </tr>
              </thead>
              <tbody>
                {filteredBundles.map((bundle) => (
                  <tr key={bundle.id} className="border-b border-warm-100 hover:bg-warm-50">
                    <td className="py-3 px-4">
                      <div>
                        <div className="font-medium text-warm-900">{bundle.name}</div>
                        <div className="text-xs text-warm-500 font-mono">{bundle.id}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div>
                        <div className="text-sm text-warm-900">{bundle.user.email}</div>
                        <div className="text-xs text-warm-600">{bundle.user.name || 'No name'}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span
                    ...
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">{bundle._count.bundleItems}</div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">{bundle._count.trustees}</div>
                    </td>
                    <td className="py-3 px-4">
                      {bundle.mode === 'time-lock' && bundle.releaseDate ? (
                        ...
                        <span className="text-sm text-warm-500">N/A</span>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-warm-900">
          ...
          {filteredBundles.length === 0 && (
            <div className="text-center py-8">
              <p className="text-warm-600">No bundles found matching your criteria.</p>
            </div>
          )}
        </Card>
      </div>
